  ...
  if (!notifications.length) {
    return (
      <div className="rounded-[2rem] bg-white/5 p-10 text-center text-brand-200 shadow-panel">
        <p className="text-xl font-semibold text-white">No notifications</p>
        <p className="mt-3 text-sm text-brand-300">You'll be notified when important events occur.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-[2rem] bg-white/5 p-6 shadow-panel">
        ...
        <p className="mt-2 text-sm text-brand-300">Stay updated with request and system events.</p>
      </div>
      <div className="space-y-4">
        {notifications.map((notification) => {
          let bgColor = 'bg-white/5 border border-white/10';
          let textColor = 'text-brand-300';
          ...
          return (
            <div key={notification._id} className={`rounded-2xl p-4 ${bgColor}`}>
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-semibold text-white">{notification.title}</h3>
                  <p className="mt-1 text-sm text-brand-300">{notification.message}</p>
                </div>
                <span
                  className={`text-xs font-medium px-2 py-1 rounded ${
                    notification.read ? 'bg-white/5 text-brand-300' : 'bg-brand-300 text-brand-950'
                  }`}
                >
