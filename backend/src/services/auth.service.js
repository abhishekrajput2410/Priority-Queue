...
const register = async ({ name, email, password, role = 'User' }) => {
  ...
  await createAuditEntry({ user: user._id, action: 'register', resource: 'User', metadata: { email } });
  return { user, message: 'Registration successful, verification email sent' };
};

const verifyEmail = async (token) => {
  const user = await User.findOne({ verificationToken: token, verificationTokenExpiry: { $gt: Date.now() } });
  if (!user) throw new Error('Verification token is invalid or expired');
  user.verified = true;
  user.verificationToken = undefined;
  user.verificationTokenExpiry = undefined;
  await user.save();
  await createAuditEntry({ user: user._id, action: 'verify_email', resource: 'User' });
  return user;
};
...
const login = async ({ email, password }) => {
  ...
  await user.save();
  await createAuditEntry({ user: user._id, action: 'login', resource: 'User' });
